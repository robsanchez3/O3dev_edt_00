/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#include <gui_generated/genupdate_screen/GenUpdateViewBase.hpp>
#include <touchgfx/Color.hpp>
#include <images/BitmapDatabase.hpp>
#include <texts/TextKeysAndLanguages.hpp>

GenUpdateViewBase::GenUpdateViewBase() :
    buttonCallback(this, &GenUpdateViewBase::buttonCallbackHandler)
{
    __background.setPosition(0, 0, 800, 480);
    __background.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    add(__background);

    si_info.setBitmap(touchgfx::Bitmap(BITMAP_INFO_100_ID));
    si_info.setPosition(680, 25, 100, 100);
    si_info.setScalingAlgorithm(touchgfx::ScalableImage::BILINEAR_INTERPOLATION);
    add(si_info);

    ta_info.setPosition(172, 75, 457, 351);
    ta_info.setColor(touchgfx::Color::getColorFromRGB(255, 255, 255));
    ta_info.setLinespacing(0);
    Unicode::snprintf(ta_infoBuffer, TA_INFO_SIZE, "%s", touchgfx::TypedText(T___SINGLEUSE_8JPS).getText());
    ta_info.setWildcard(ta_infoBuffer);
    ta_info.setTypedText(touchgfx::TypedText(T___SINGLEUSE_CH0B));
    add(ta_info);

    bt_cancel.setXY(10, 370);
    bt_cancel.setBitmaps(touchgfx::Bitmap(BITMAP_CANCELBUTTON_01_A_ID), touchgfx::Bitmap(BITMAP_CANCELBUTTON_01_B_ID));
    bt_cancel.setAction(buttonCallback);
    add(bt_cancel);

    bt_OK.setXY(690, 370);
    bt_OK.setBitmaps(touchgfx::Bitmap(BITMAP_ENTERBUTTON_02_A_ID), touchgfx::Bitmap(BITMAP_ENTERBUTTON_02_B_ID));
    bt_OK.setAction(buttonCallback);
    add(bt_OK);

    si_icon.setBitmap(touchgfx::Bitmap(BITMAP_SETTINGS_100_A_ID));
    si_icon.setPosition(20, 25, 100, 100);
    si_icon.setScalingAlgorithm(touchgfx::ScalableImage::BILINEAR_INTERPOLATION);
    add(si_icon);
}

GenUpdateViewBase::~GenUpdateViewBase()
{

}

void GenUpdateViewBase::setupScreen()
{

}

void GenUpdateViewBase::buttonCallbackHandler(const touchgfx::AbstractButton& src)
{
    if (&src == &bt_cancel)
    {
        //cancelClicked1
        //When bt_cancel clicked call virtual function
        //Call cancelClicked
        cancelClicked();
    }
    if (&src == &bt_OK)
    {
        //okClicked1
        //When bt_OK clicked call virtual function
        //Call okClicked1
        okClicked1();
    }
}
