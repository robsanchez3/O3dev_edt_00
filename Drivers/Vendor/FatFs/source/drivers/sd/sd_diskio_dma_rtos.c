    ...
    if ((status == osOK) && (event == READ_CPLT_MSG))
    {
      ...
      while(osKernelGetTickCount() - timer < SD_TIMEOUT)
      {
        if (HAL_SD_GetCardState(&sdmmc_handle) == HAL_SD_CARD_TRANSFER)
        {
           ...
           */
          HAL_DCACHE_InvalidateByAddr(&hdcache1,(uint32_t*)buff, count*BLOCKSIZE);
#endif
          break;
        }
      }
...
static DRESULT SD_DMA_write(BYTE lun, const BYTE *buff, LBA_t sector, UINT count)
{
  ...
  */
	  HAL_DCACHE_CleanByAddr(&hdcache1,(uint32_t*)buff, count*BLOCKSIZE);
#endif
  ...
  else
  {
    ...
    for (i = 0; i < count; i++)
    {
      ...
      HAL_DCACHE_CleanByAddr(&hdcache1,(uint32_t*)scratch, BLOCKSIZE);
